function __print_lime_functions_help() {
cat <<EOF
...
- limegerrit:   A Git wrapper that fetches/pushes patch from/to Lime Gerrit Review.
- limerebase:   Rebase a Gerrit change and push it again.
- limeremote:   Add git remote for Lime Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for LIME Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Lime model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch lime_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/lime-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.lime.device | grep -q "$LIME_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $LIME_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function limeremote()
{
    ...
    local LIME_USER=$(git config --get review.review.lime.com.username)
    if [ -z "$LIME_USER" ]
    then
        git remote add lime ssh://review.lime.com:29418/$PFX$PROJECT
    else
        git remote add lime ssh://$LIME_USER@review.lime.com:29418/$PFX$PROJECT
    fi
    echo "Remote 'lime' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/LimeProject/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.lime.device | grep -q "$LIME_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        limeremote
        git push lime HEAD:refs/heads/'$1'
    fi
    '
}
...
function limegerrit() {
    ...
    local user=`git config --get review.review.lime.com.username`
...
function limerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "LIME Gerrit Rebase Usage: "
        echo "      limerebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.lime.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lime.device | grep -q "$LIME_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/lime/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LIME_FIXUP_COMMON_OUT ]; then
