...
func init() {
	android.RegisterModuleType("lime_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = limeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = limeExpandVariables(ctx, dep_file)
	...
	cmd := limeExpandVariables(ctx, String(g.properties.Cmd))
